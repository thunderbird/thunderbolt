...
- **Cross-device propagation.** Imports write through the same path as any other local change to a synced table: the new rows are queued in PowerSync's CRUD log and uploaded to the backend, which fans them out to every other device the user is signed in to. Restoring an old backup overwrites newer rows everywhere they share an ID, not just on the device running the import. The confirm dialog spells this out.
