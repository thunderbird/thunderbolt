...
import { APIConnectionError as AnthropicAPIConnectionError, APIError as AnthropicAPIError } from '@anthropic-ai/sdk'
...
export const classifyInferenceError = (error: unknown): InferenceErrorKind => {
  if (error instanceof APIConnectionError || error instanceof AnthropicAPIConnectionError) {
    return 'connection'
  }
  ...
  const providerCode = error instanceof APIError ? error.code : error.type
  const codeKind = providerCode ? providerErrorKinds[providerCode.toLowerCase()] : undefined
  if (codeKind) {
    return codeKind
  }
