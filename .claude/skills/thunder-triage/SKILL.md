...
Invoke with `/thunder-triage <findings-path> [--auto] [--votes N] [--repo PATH] [--fp-rules FILE]`.
...
Phase state persists to `./.triage-state/` so a fresh `/thunder-triage` session can
...
```
/thunder-triage .claude/skills/triage/fixtures/canary-findings.json --auto --repo targets/canary
```
...
/thunder-triage results/drlibs/<ts>/ --repo targets/drlibs
```
