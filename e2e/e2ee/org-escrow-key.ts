 ...
 * with `bun scripts/org-escrow-keygen.ts --json`. The public half is the escrow
 * PIN for the harness frontend build (VITE_ORG_ESCROW_PUBLIC_KEY in
 * playwright.e2ee.config.ts); the private half lets org-escrow.spec.ts and
 * attacks/org-key-substitution.spec.ts run the offline decrypt tool.
