...
description: Thunderbolt's zero-knowledge E2E encryption — key hierarchy, device approval, recovery.
---

import { Aside } from '@astrojs/starlight/components';

<Aside type="caution">
End-to-end encryption is in **Preview**. It has not yet undergone a cryptography audit and is subject to further refinements. Canonical source: [`docs/e2e-encryption.md`](https://github.com/thunderbird/thunderbolt/blob/main/docs/e2e-encryption.md).
</Aside>
...
The download and upload middleware both read from `encryptedColumnsMap` in [`src/db/encryption/config.ts`](https://github.com/thunderbird/thunderbolt/blob/main/src/db/encryption/config.ts) — a single source of truth for which columns are encrypted.
...
To encrypt a new column, add the table and column name to `encryptedColumnsMap` in [`src/db/encryption/config.ts`](https://github.com/thunderbird/thunderbolt/blob/main/src/db/encryption/config.ts). The existing `encryptionMiddleware` handles every column in the map automatically — both download decryption and upload encryption.
...
Encryption is implemented as a PowerSync transform-middleware. On **Chrome/Edge/Firefox** it runs inside a custom SharedWorker so the CK stays in one place across tabs; on **Safari and Tauri** it runs on the main thread because those environments don't support SharedWorker. See [Multi-Device Sync](/docs/guides/multi-device-sync#two-sync-paths) and [`docs/powersync-sync-middleware.md`](https://github.com/thunderbird/thunderbolt/blob/main/docs/powersync-sync-middleware.md) for the full architecture.
