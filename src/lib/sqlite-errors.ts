...
export const isInsertConflictError = (error: unknown): boolean => {
  ...
  if (typeof err.errno === 'number' && insertConflictCodes.has(err.errno)) {
    return true
  }
  if (typeof err.code === 'string' && /SQLITE_CONSTRAINT_(UNIQUE|PRIMARYKEY|ROWID)/.test(err.code)) {
    return true
  }
