...
test.slow()
test.skip(process.env.E2E_NIGHTLY_WEBKIT_PERSISTENT === 'true', 'PowerSync runs only in the Linux Nightly')
...
test('a chat created on one device appears on another without reloading', async ({ page, browser, baseURL }) => {
  if (!baseURL) throw new Error('Nightly sync project needs a baseURL')
