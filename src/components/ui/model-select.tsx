...
export const ModelSelect = memo(({ chatThread, models, selectedModelId, onModelChange }: ModelSelectProps) => {
  ...
  const renderTrigger = (selected: SearchableMenuItem<ModelItemData> | undefined, isOpen: boolean) => (
    <div
      className={cn(
        'flex items-center justify-between gap-2 px-3 py-1.5 rounded-xl cursor-pointer transition-colors text-[length:var(--font-size-body)] border h-[var(--touch-height-default)] min-w-[140px]',
