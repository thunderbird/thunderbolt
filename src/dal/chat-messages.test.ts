...
describe('Chat Messages DAL', () => {
  ...
  // NOTE: Database-level cascade deletes via foreign key constraints are no longer supported.
  // For cr-sqlite CRR compatibility, cascade deletes must be handled in application code.
  // The parentId column is now a logical reference only, not an enforced FK constraint.
})
