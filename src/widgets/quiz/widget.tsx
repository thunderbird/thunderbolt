...
export const QuizWidget = ({ prompt, mode, options, explanation, messageId }: QuizWidgetProps) => {
  ...
  const handleSubmit = async ({ selectedIds, correct }: QuizSubmission) => {
    ...
    // Persist the choice so the quiz restores answered on reload. We intentionally
    // do NOT dispatch a follow-up turn: the widget already grades and reveals the
    // answer client-side, so a question stands on its own, and the persisted entry
