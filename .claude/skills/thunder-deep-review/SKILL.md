...
- **C. Conventions / house rules** — `references/house-rules.md` (TS + React + data). camelCase even for constants, no `let`, braces on all ifs, `@/` imports, `.test`+`bun:test`, direct imports.
...
- **E. Error handling** — caught error downgraded to `console.warn`/silent return on a path that shouldn't fail → "let it throw"; error/early-return branch with **no** logging → "at least `console.error`"; over-defensive guard on trusted data; `setTimeout`/`requestAnimationFrame` papering over a race/ordering bug.
- **F. Testability** — heavy mocking (esp. `mock.module`/`vi.mock` of internal collaborators) = modularity smell → DI; branchy logic without unit tests; `.spec`/`vitest`; `as any`/`as unknown as`.
