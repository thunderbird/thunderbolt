...
description: How Thunderbolt keeps your conversations in sync across every device, offline-first.
---
...
<Aside type="note">
Cross-device sync and optional end-to-end encryption are both in **Preview**. See the roadmap for current status.
</Aside>
...
- A transform-middleware pipeline sits between PowerSync and SQLite. The built-in `encryptionMiddleware` decrypts encrypted columns on download and encrypts them on upload. See [End-to-End Encryption](/docs/guides/end-to-end-encryption).
...
The canonical write-up is [`docs/powersync-sync-middleware.md`](https://github.com/thunderbird/thunderbolt/blob/main/docs/powersync-sync-middleware.md), including the Vite alias (`powersync-web-internal`) that lets the custom SharedWorker reach into `@powersync/web`'s `@internal` classes.

## Synced tables

From [`shared/powersync-tables.ts`](https://github.com/thunderbird/thunderbolt/blob/main/shared/powersync-tables.ts):
...
Default-data tables use composite primary keys so multiple users can hold the same default id — see [Contributing](/docs/guides/contributing#composite-primary-keys).
...
1. **Backend + sync rules PR**
   ...
   - Register in [`shared/powersync-tables.ts`](https://github.com/thunderbird/thunderbolt/blob/main/shared/powersync-tables.ts) (`POWERSYNC_TABLE_NAMES` + `powersyncTableToQueryKeys`).
...
Why: the backend is a sync server, not a query engine. Heavy queries run on the client's SQLite. Indexes on encrypted columns would be useless anyway, and fewer indexes means faster writes during sync. Full rationale in [`docs/composite-primary-keys-and-default-data.md`](https://github.com/thunderbird/thunderbolt/blob/main/docs/composite-primary-keys-and-default-data.md).

## Related reading

- [Account & Devices](/docs/guides/account-and-devices) — how devices are registered and revoked.
- [End-to-End Encryption](/docs/guides/end-to-end-encryption) — how the sync pipeline encrypts data before it reaches the server.
- [Contributing](/docs/guides/contributing) — schema rules, composite keys, and the migration checklist.
