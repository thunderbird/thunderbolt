...
describe('exportFilenameFor', () => {
  it('formats the filename as YYYY-MM-DD in local time', () => {
...
describe('downloadJson', () => {
  ...
  it('serializes the payload as pretty-printed JSON', async () => {
    downloadJson('export.json', { hello: 'world', list: [1, 2, 3] })
    const blob = createSpy.mock.calls[0]?.[0] as Blob
    const text = await blob.text()
    expect(JSON.parse(text)).toEqual({ hello: 'world', list: [1, 2, 3] })
