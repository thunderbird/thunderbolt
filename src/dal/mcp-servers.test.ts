...
describe('MCP Servers DAL', () => {
  ...
  describe('getHttpMcpServers', () => {
    it('should return empty array when no HTTP servers exist', async () => {
      ...
      const servers = await getHttpMcpServers(getDb(), wsId)
      expect(servers).toEqual([])
    })

    it('should return only HTTP servers with URLs in the workspace', async () => {
      const db = getDb()
      const serverId1 = uuidv7()
      const serverId2 = uuidv7()
    ...
    it('should not return HTTP MCP servers from other workspaces', async () => {
      const db = getDb()
      const ownId = uuidv7()
      const otherId = uuidv7()
  ...
  describe('deleteMcpServer', () => {
    ...
    it('should not return soft-deleted server via getHttpMcpServers', async () => {
      ...
      const serversBefore = await getHttpMcpServers(getDb(), wsId)
      expect(serversBefore).toHaveLength(1)

      await deleteMcpServer(getDb(), wsId, serverId)

      // Verify server is not returned after soft deletion
      const serversAfter = await getHttpMcpServers(getDb(), wsId)
      expect(serversAfter).toHaveLength(0)
    })
  ...
  describe('createMcpServer', () => {
    ...
    it('should create an HTTP server that appears in getHttpMcpServers', async () => {
      ...
      const httpServers = await getHttpMcpServers(getDb(), wsId)
