...
export const ErrorMessage = memo(({ retryCount, retriesExhausted, onRetry }: ErrorMessageProps) => {
  ...
  if (retryCount > 0 && !retriesExhausted) {
    return (
      <div className="px-[var(--spacing-x-default)] py-[var(--spacing-x-md)] rounded-[var(--radius-2xl)] bg-amber-500/10 border border-amber-500/20 mr-auto w-full mt-2">
  ...
  return (
    <div className="px-[var(--spacing-x-default)] py-[var(--spacing-x-md)] rounded-[var(--radius-2xl)] bg-destructive/10 border border-destructive/20 mr-auto w-full mt-2">
      <div className="flex items-center justify-between gap-[var(--gap-lg)] min-h-[var(--touch-height-sm)]">
        ...
        {onRetry && (
          <button
            type="button"
            onClick={onRetry}
            className="cursor-pointer shrink-0 text-[length:var(--font-size-body)] font-medium text-destructive/90 bg-destructive/10 hover:bg-destructive/15 px-[var(--spacing-x-md)] py-[var(--spacing-y-sm)] rounded-[var(--radius-xl)]"
          >
            Retry
          </button>
        )}
