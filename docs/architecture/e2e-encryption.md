...
- **Client:** local key material is the authority. `needsSyncSetupWizard()` (`src/db/encryption/config.ts`) returns `true` until an AK plus at least one wrapped DEK exist locally, and `codec.encode` fails closed whenever setup has completed or an AK is present. The connector's canary probe (`GET /encryption/canary`) distinguishes a genuinely pre-E2EE account (404 → plaintext passthrough by design) from an unprovable state (offline/5xx → uploads deferred, download credentials withheld). An auth-coded rejection on the probe additionally surfaces re-authentication (the probe result stays `unknown`, so the fail-closed rule is untouched) — without that, a device with an expired or revoked session would silently retry a dead session forever instead of showing the sign-in prompt.
...
- **`key_id`** — names the DEK that encrypted this value. This is what makes DEK rotation free: every value permanently remembers its own key, so any device can pick the right DEK out of the keyring at read time, no matter how many times the primary changed since. The parser validates it against the complete legitimate set (`isWireKeyId`: mintable ids plus the reserved `"v1"` slot) — every path that creates a key_id enforces that grammar, so a label outside it is server-invented by construction and dies at parse, instead of driving key-request traffic for a key that can't exist. A rejected value stays opaque: it still carries the `__enc:` prefix, so it can never be mistaken for plaintext.
