...
describe('Google buildAuthUrl', () => {
  ...
  it('throws MisconfiguredOAuthError with both-missing message when client_id is empty', async () => {
    ...
    expect(err.message).toContain('GOOGLE_CLIENT_ID and GOOGLE_CLIENT_SECRET')
  })
  ...
  it('throws MisconfiguredOAuthError with secret-only message when client_id is set but configured=false', async () => {
    ...
    expect(err.message).toContain('Set GOOGLE_CLIENT_SECRET')
    expect(err.message).not.toContain('GOOGLE_CLIENT_ID and')
  })
