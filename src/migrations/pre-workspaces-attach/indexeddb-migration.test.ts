...
type Op = { kind: 'read' | 'write' | 'delete'; dbName: string }
...
describe('migrateEncryptionKeysIfNeeded', () => {
  it('copies all 5 legacy key-store entries into the namespaced DB and deletes the legacy DB', async () => {
    ...
    expect(dbs.has(legacyDbName)).toBe(false)
  ...
  it('is a no-op when the legacy DB has no entries — still cleans up the empty legacy DB stub', async () => {
    ...
    // Even with nothing to migrate, we still issue the delete so any empty stub
    // (e.g. one that openExisting accidentally created in production IDB) is
    // cleaned up.
    expect(ops.find((o) => o.kind === 'delete' && o.dbName === legacyDbName)).toBeDefined()
  })
  ...
  it('reports migrated=false when every legacy entry was already in the new DB', async () => {
    ...
    // skip every write but still drop the legacy DB.
    const { backend, dbs } = makeFakeBackend({
      [legacyDbName]: legacyKeys,
  ...
  it('orders ops: read legacy → write namespaced → delete legacy', async () => {
    ...
    expect(ops).toEqual([
      ...
      { kind: 'delete', dbName: legacyDbName },
    ])
  })
  ...
  it('namespaces under the given serverId — running against another server is independent', async () => {
    ...
    // Legacy DB is gone after A's run; B has nothing to migrate.
    const resultB = await migrateEncryptionKeysIfNeeded(serverB, backend)
    expect(resultB).toEqual({ migrated: false, entryCount: 0 })
    expect(dbs.has(namespacedDbName(serverB))).toBe(false)
  })
