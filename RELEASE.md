...
```bash
# Auto-detect version from commits (defaults to --type auto)
bun run scripts/create-release.ts

# Review changes, then push when ready
git push origin main
git push origin v1.2.3

# OR do it all in one step (auto-detect and push)
...
```bash
# Auto-detect based on conventional commits (default behavior)
gh workflow run create-version-tag.yml

# Or explicitly specify bump type
gh workflow run create-version-tag.yml -f version_type=minor  # or major, patch, auto
```
