...
4. ✅ Build Desktop, iOS, and Android simultaneously
   ...
   - Android increments versionCode during build
...
| `bundle.android.versionCode` (tauri.conf)    | **android-release.yml** 🤖 | Android build number (auto-inc)  |
...
2. **Platform-specific versions**:
   ...
   - **Android**: `versionCode` auto-incremented by `android-release.yml` (committed)
...
2. **Platform Builds** (runs in parallel):
   ...
   - **Android**: Increments `versionCode`, builds, and uploads to Play Store (Internal)
...
1. **Version Bump Job**
   - Updates all version files: `package.json`, `Cargo.toml`, `tauri.conf.json`, iOS `project.yml`
