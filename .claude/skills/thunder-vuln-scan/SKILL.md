...
description: >-
  ...
  area, and writes VULN-FINDINGS.json + .md for /thunder-triage to consume. Read-only
  ...
  /thunder-threat-model and /thunder-triage.
...
(+ a human-readable `.md`) that `/thunder-triage` ingests directly.
...
2. **Light dedupe** — if two findings cite the same `file:line` with the
   ...
   duplicate id. (Heavy dedupe is `/thunder-triage`'s job; don't over-engineer here.)
...
`/thunder-triage` see high-signal findings first. Spawn **one Task subagent per
...
**`VULN-FINDINGS.json`** — the `/thunder-triage` ingest shape:
...
3. Next step: `> /thunder-triage <target-dir>/VULN-FINDINGS.json --repo <target-dir>`
...
- Findings are candidates for `/thunder-triage`, not final verdicts. **This skill
  never drops a finding** — Step 3b only ranks. `/thunder-triage` does the rigorous
