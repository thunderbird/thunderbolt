...
**Full documentation** (synced tables, adding tables, account/device flows, backend API): [docs/architecture/powersync-account-devices.md](../docs/architecture/powersync-account-devices.md).
...
3. **PowerSync env vars** in `backend/.env`: see [docs/architecture/powersync-account-devices.md](../docs/architecture/powersync-account-devices.md) (section 3). The config in `config/config.yaml` uses HS256 with the same secret/kid so backend-issued tokens are accepted.
