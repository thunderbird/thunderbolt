...
| `THUNDERBOLT_BRIDGE_HOST`                    | Interface the wss bridge binds to (default `127.0.0.1`). Set to `0.0.0.0` only for a deployed agent, which also requires a stable token. A public bind drops the `Origin` check, and a browser can only reach a public TLS bridge — see [docs/hosted-agent.md](docs/hosted-agent.md).                      |
