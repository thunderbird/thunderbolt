...
import { ecdsaKeyAlgorithm, encodeChallengePayload, type ChallengeOperation } from '@shared/e2ee-types'
...
/** Pinned AAD for the canary — create and verify must always agree (decided once, SF1/B4). */
export const canaryAAD = new TextEncoder().encode('thunderbolt-canary-v2')
...
/**
 * Create a canary by encrypting a known prefix + random secret under the DEK
 * with key_id "0" (`initialKeyId` — retained forever, so verify always works),
 * bound to the pinned `canaryAAD`.
 */
export const createCanary = async (dek0: CryptoKey): Promise<Canary> => {
  const canarySecret = generateCanarySecret()
  const plaintext = `${canaryPrefix}:${canarySecret}`
  const { iv, ciphertext } = await encrypt(plaintext, dek0, canaryAAD)
  return { canaryIv: iv, canaryCtext: ciphertext, canarySecret }
}

/**
 * Verify key material by decrypting the canary (AAD-bound) and comparing to the
 * known prefix. Returns the embedded secret on success — it seeds the
 * deterministic signing keypair for challenge-response proofs.
 */
export const verifyCanary = async (
  ...
  try {
    const decrypted = await decrypt({ iv: canaryIv, ciphertext: canaryCtext }, dek0, canaryAAD)
