...
A binary knowledge base would therefore work only on the device that uploaded it
and vanish when IndexedDB is cleared. So knowledge is **extracted text**: uploads
...
Order is the safety mechanism: `getProjectFiles` sorts user content (uploads and
typed notes) ahead of assistant-written notes, so **anything the assistant wrote
...
| Knowledge import + MIME resolution | `src/projects/{import-knowledge-files,extract-knowledge-text}.ts` |
| Pages | `src/projects/{index,project-detail,create-project-panel}.tsx` |
