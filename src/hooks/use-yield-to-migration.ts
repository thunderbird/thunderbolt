...
type UseYieldToMigrationOptions = {
  ...
  /** Called to close the competing surface. */
  close: () => void
}

/**
 * Close a blocking setup surface when the seamless v1→v2 migration finishes,
 * so only one recovery-phrase dialog is ever on screen.
 *
 ...
 */
export const useYieldToMigration = ({ open, isShowingOwnRecoveryKey, close }: UseYieldToMigrationOptions): void => {
  useEffect(() => {
    if (!open || isShowingOwnRecoveryKey) {
      return
    }
    const yieldToMigration = () => close()
  ...
  }, [open, isShowingOwnRecoveryKey, close])
}
